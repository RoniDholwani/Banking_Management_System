...
public class AccountManager {
    ...
    // Add these methods in AccountManager.java
    ...
    public boolean debit_moneyGUI(long account_number, double amount, String security_pin) {
        try {
            connection.setAutoCommit(false);
            PreparedStatement ps = connection.prepareStatement("SELECT * FROM Accounts WHERE account_number = ? and security_pin = ?");
            ps.setLong(1, account_number);
    ...
    public boolean transfer_moneyGUI(long sender_account_number, long receiver_account_number, double amount, String security_pin) {
        try {
            connection.setAutoCommit(false);
            PreparedStatement ps = connection.prepareStatement("SELECT * FROM Accounts WHERE account_number = ? AND security_pin = ?");
            ...
            if (rs.next()) {
                double balance = rs.getDouble("balance");
                if (amount <= balance) {
                    PreparedStatement debit = connection.prepareStatement("UPDATE Accounts SET balance = balance - ? WHERE account_number = ?");
                    PreparedStatement credit = connection.prepareStatement("UPDATE Accounts SET balance = balance + ? WHERE account_number = ?");
